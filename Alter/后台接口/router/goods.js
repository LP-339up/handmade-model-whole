...
var goodsList=function(req,res){
    ...
    var pageSize=req.query.pageSize || req.body.pageSize || 8
    ...
    var sql=`SELECT
         id, name, status, goodsImg, attr,createTime
          ...
          if(start){
            sql+='AND UNIX_TIMESTAMP(createTime) > UNIX_TIMESTAMP("'+start+'")'
          }
           if(end){
            sql+=' AND UNIX_TIMESTAMP(createTime) < UNIX_TIMESTAMP("'+end+'")'
           } 
...
var goodsList1=function(req,res){
    //多表查询  分页limit   排序order by
    var name =req.query.name||req.body.name
    var createTime=getTime(req.body.createTime)
    //需要前端传入的数据 currentPage 当前在多少页 pageSize 每一个页面放的数据长度
    var currentPage=req.query.current || req.body.current || 1
    var pageSize=req.query.pageSize || req.body.pageSize || 8
    ...
    var sql=`SELECT
            *
            FROM commodity_details where UNIX_TIMESTAMP(DATE) < UNIX_TIMESTAMP(?)
            `
        if(name){
             sql+='and status = 1 and name like "%'+name+'%"'
        }   
        sql+=`ORDER BY DATE DESC LIMIT ?,?`  
    var data=[createTime,(currentPage-1)*pageSize,Number(pageSize)]
    query(sql,data).then(function(result){
        var sql1=`SELECT
                *
                FROM commodity_details where UNIX_TIMESTAMP(DATE) < UNIX_TIMESTAMP(?)
        ...
        query(sql1,createTime).then(function(resl){
            res.json({
                status:200,
                list:result,
                total:resl.length
...
var getHeatgoods=function(req,res){
    query('SELECT name,price,goodsImg,attr FROM commodity_details ORDER BY heat DESC LIMIT 7').then(function(result){
        res.json({
            status:200,
            list:result
        })
